...
class JADNPackage:
    def __init__(self, root, meta: dict = None):
        self.meta = {}
        self.types = []
        self.root = root
        ...
        # --- Fix: Ensure at least one type is present ---
        if not self.types:
            self.types.append([
                "Placeholder",
                "Record",
                [],
                "Placeholder type to satisfy minItems=1",
                []
            ])                    
...
def xsd_to_jadn(root: etree.Element) -> JADNPackage:
    ...
    meta['namespaces'] = [[k if k is not None else '', v] for k, v in root.nsmap.items()]  # Fix: no None prefix
